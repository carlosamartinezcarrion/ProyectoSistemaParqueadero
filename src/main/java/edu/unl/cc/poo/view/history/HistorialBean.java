...
public class HistorialBean implements Serializable {
    ...
    public String exportarCsv() {
        ...
        for (Registro registro : registros) {
            csv.append(valor(registro.getId())).append(',')
    ...
    private List<Registro> aplicarFiltros() {
        return parqueaderoService.getHistorial().stream()
                .filter(registro -> placaFiltro == null || placaFiltro.isBlank()
                        || registro.getVehiculo().getPlaca().toLowerCase().contains(placaFiltro.toLowerCase()))
                .filter(registro -> tipoFiltro == null || tipoFiltro.isBlank()
                        || registro.getVehiculo().getTipo().name().equals(tipoFiltro))
                .filter(registro -> fechaFiltro == null || registro.getFechaHoraEntrada().toLocalDate().equals(fechaFiltro))
