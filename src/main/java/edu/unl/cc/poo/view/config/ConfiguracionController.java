package edu.unl.cc.poo.view.config;
...
public class ConfiguracionController implements Serializable {
    ...
    private void cargarTarifas() {
        ...
        List<Tarifa> tarifas = getConfiguracion().getTarifas();
        for (TipoVehiculo tipo : TipoVehiculo.values()) {
            Tarifa t = tarifas.stream()
                    .filter(tar -> tar.getTipoVehiculo() == tipo)
                    .findFirst()
                    .orElse(null);
            if (t != null) {
    ...
    public List<TarifaEditable> getTarifasEditables() {
        if (tarifasEditables == null) cargarTarifas();
    ...
    public void subirLogo() {
        ...
        if (fileName == null || fileName.isBlank()) {
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error", "Nombre de archivo invalido."));
            return;
        }
        String ext = "";
        ...
        try {
            ...
            FacesContext.getCurrentInstance().addMessage(null,
                    new FacesMessage(FacesMessage.SEVERITY_ERROR, "Error al subir logo", ex.getMessage()));
        }
    }
    ...
    public void setFilasDefecto(int v) { this.filasDefecto = v; }
    public int getColumnasDefecto() { return columnasDefecto; }
    public void setColumnasDefecto(int v) { this.columnasDefecto = v; }
    public Part getLogoFile() { return logoFile; }
    public void setLogoFile(Part v) { this.logoFile = v; }
    ...
    public static class TarifaEditable implements Serializable {
        ...
        @DecimalMin(value = "1", message = "La fracción de minutos debe ser al menos 1")
        ...
        public void setFraccionMinutos(double v) { this.fraccionMinutos = v; }
    }
}
